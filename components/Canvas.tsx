...
const Canvas: React.FC<canvasProps> = ({
  ...
  return (
    <section className="space-y-4 w-full flex flex-row justify-between gap-5">
      <section className="flex flex-col justify-between h-full gap-5">
            ...
            {selectedTexts?.map((textbox) => (
              <Box
                className={`flex flex-row justify-around items-end space-x-2 ${
                  selectedId.textbox === textbox.id ? "absolute top-6 " : ""
                }`}
                key={textbox.id}
              >
